...
function mn_get_second_checksum_index(words, wordset_length, prefix_len) {
    var trimmed_words = "";
    for (var i = 0; i < words.length; i++) {
        trimmed_words += words[i].slice(0, prefix_len);
    }
    var checksum = crc32.run(trimmed_words);
    return checksum % wordset_length;
}
...
function mn_encode(str, wordset_name) {
    ...
    if (wordset.prefix_len > 0 && str.length > 0) {
        out.push(out[mn_get_checksum_index(out, wordset.prefix_len)]);
        out.push(wordset.words[mn_get_second_checksum_index(out, wordset.words.length, wordset.prefix_len)]);
    }
    return out.join(' ');
}
...
function mn_decode(str, wordset_name) {
    ...
    var checksum_word = '';
    var second_checksum_word = '';
    if (wlist.length < 24 || wlist.length > 26)
        throw "You've entered too few or too many words, please try again";
    if (wlist.length == 26) {
        // Pop second checksum from mnemonic
        var word = wlist.pop();
        if (typeof word !== 'undefined')
            second_checksum_word = word;
        // Pop checksum from mnemonic
        var word2 = wlist.pop();
        if (typeof word2 !== 'undefined')
            checksum_word = word2;
    }
    else if (wlist.length == 25) {
        // Pop checksum from mnemonic
        var word = wlist.pop();
        if (typeof word !== 'undefined')
            checksum_word = word;
    ...
    if (checksum_word != '') {
        ...
        if (second_checksum_word != '') {
            ...
            var index_1 = mn_get_second_checksum_index(wlist, wordset.words.length, wordset.prefix_len);
            var expected_second_checksum_word = wordset.words[index_1];
            if (expected_second_checksum_word.slice(0, wordset.prefix_len) !== second_checksum_word.slice(0, wordset.prefix_len)) {
                throw "Your private key could not be verified, please try again";
            }
        }
    }
